...
def save_filter_to_wav(
    ...
    if sample_format == "pcm16":
        ...
        raise ValueError("Unsupported sample format. Use 'float32' or "
                         "'pcm16'.")


def _create_multichannel_data(data: np.ndarray,
...
def _progress_iter(
    ...
    try:
        ...
        print("\r" + f"{desc} [{'#'*width}] {total}/{total} (100.0%)" +
              " " * max(0, term_cols - len(desc) - width - 20), flush=True)
        print()
