...
def apply_db_smoothing(db_values: np.ndarray,
...
def compute_relative_gains(
    ...
    """
    Compute relative gains needed to transform from source to target phon
    level.
    ...
    if source_key not in target_curves:
        raise ValueError(
            f"Source phon level {source_key} not available in curves")
    if target_key not in target_curves:
        raise ValueError(
            f"Target phon level {target_key} not available in curves")
    ...
    if reference_index >= len(db_diff):
        raise ValueError(f"Reference index {reference_index} out of bounds for "
                         f"db_diff length {len(db_diff)}")
    ...
    if smooth_db and smooth_window > 1:
        relative_gains_db = apply_db_smoothing(relative_gains_db,
                                               smooth_window)
...
def prepare_target_response(
    ...
    gain_nodes_linear = np.array([dc_gain] + relative_gains_linear.tolist() +
                                 [nyq_gain], dtype=float)
    ...
    interpolator = PchipInterpolator(iso_nodes_hz, gain_nodes_linear,
                                    extrapolate=False)
...
def design_fir_filter_from_phon_levels(
    ...
    """
    Design an FIR filter to transform equal-loudness curve between phon
    levels.
    ...
    # Design FIR filter
    fir_coefficients = np.array(signal.firwin2(config.numtaps, freqs, gains,
                                               fs=config.fs))
